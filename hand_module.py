...
class HandDetector:
    ...
    def fingers_up(self):
        if len(self.land_mark_list)!= 0:
            ...
            fingers_counter=[]
            if self.land_mark_list[self.tips_id[0]][1] > self.land_mark_list[self.tips_id[0] - 1][1]:
                fingers_counter.append(1)
            else:
                fingers_counter.append(0)
            for tip_id in range(1,5):
                if self.land_mark_list[self.tips_id[tip_id]][2]<self.land_mark_list[self.tips_id[tip_id] - 2][2]:
                    fingers_counter.append(1)
                else:
                     fingers_counter.append(0)
